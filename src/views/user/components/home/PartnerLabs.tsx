...
export function PartnerLabs({ initialLabs }: { initialLabs?: any }) {
  const labs = Array.isArray(initialLabs?.data) 
    ? initialLabs.data 
    : (Array.isArray(initialLabs) ? initialLabs : (initialLabs?.data?.data || []));
          ...
          {labs.length > 0 ? (
            labs.slice(0, 6).map((lab: any, i: number) => {
              ...
              return (
                <div 
                  key={lab._id || i} 
                  className="group flex flex-col items-center justify-center p-8 rounded-[2rem] border-2 border-slate-100 bg-slate-50/50 transition-all duration-300 hover:bg-white hover:border-brand-primary/30 hover:shadow-[0_20px_40px_-15px_rgba(0,117,31,0.12)]"
                >
                  ...
                  <p className="mt-2 text-center text-[10px] font-bold uppercase tracking-widest text-brand-primary/80">
                    {acc}
                  </p>
                </div>
              );
