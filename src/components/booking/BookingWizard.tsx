    ...
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <div className={wizard.step === 5 ? "col-span-12" : "lg:col-span-8"}>
          {wizard.step === 0 && <StepSampleScope items={wizard.items} onToggleParam={handleToggleParam} />}
          {wizard.step === 1 && <StepSampleDetails items={wizard.items} onUpdateSample={handleUpdateSample} onAddSample={handleAddSample} onRemoveSample={handleRemoveSample} />}
          {wizard.step === 2 && <StepSelectLab labs={wizard.eligibleLabs} selectedLab={wizard.selectedLab} onSelectLab={wizard.setSelectedLab} calculateLabPricing={wizard.calculateLabPricing} />}
          {wizard.step === 3 && <StepAddressSchedule formData={wizard.formData} onChange={(f, v) => wizard.setFormData(prev => ({ ...prev, [f]: v }))} />}
          {wizard.step === 4 && <StepPayment paymentMethod={wizard.paymentMethod} onSelectPaymentMethod={wizard.setPaymentMethod} total={wizard.total} />}
          {wizard.step === 5 && <StepConfirmation booking={wizard.createdBooking} />}
        </div>
